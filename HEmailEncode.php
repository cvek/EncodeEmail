...
//-->\n </script>';
  ...
  private static function processEmailInText($innerText, $t=1) {
    if ($innerText != "") {
      // Из текста без a вырезать оставшиеся e-mail-ы
      preg_match_all('/'.self::getMailPattern(false, false).'(?=[^>]*?<)/s', $innerText, $newMatches, PREG_PATTERN_ORDER);
      $c = 0;
      ...
      for ($i = 0; $i < $c; $i++) {
        $mail = $newMatches[0][$i];
        if (!self::checkEmail($mail)) continue;
        $encode = self::getJsEmail($mail);
        $encode = self::getJs('document.write('.$encode.');');
        $innerText = str_replace($mail, $encode, $innerText);
      }
    }
    return $innerText;
  }
